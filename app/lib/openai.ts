...
export const openaiAnalysis = async (question: string, context: string) => {
  const completion = await openai.chat.completions.create({
    ...
    max_tokens: 250, // Prevents the response from running too long
    messages: [
      {
        role: "system",
        content: `
You are a technical recruiter reviewing a candidate's resume based on the provided context. 

### Core Rule:
1. **Length Constraint:** Keep your answer concise and straight to the point. **Do not exceed 150 words total.**
